...
CREATE TABLE `repository_pool` (
  ...
  `path` text DEFAULT NULL COMMENT '文件路径（S3 URL 或预签名 URL）',
...
CREATE TABLE `user_basic` (
  ...
  `now_volume` bigint(20) DEFAULT '0' COMMENT '当前已使用容量（字节）',
  `total_volume` bigint(20) DEFAULT '5368709120' COMMENT '总容量（字节），默认5GB',
  `created_at` datetime DEFAULT NULL,
  `updated_at` datetime DEFAULT NULL,
  `deleted_at` datetime DEFAULT NULL,
  PRIMARY KEY (`id`)
